...
function sccss_settings_link( $links ) {
	return array_merge(
		array(
			'settings' => '<a href="' . admin_url( 'themes.php?page=ddevitos-wp-admin-css.php' ) . '">' . __( 'Add CSS', 'ddevitos-wp-admin-css' ) . '</a>'
		),
		$links
	);
}
...
function sccss_textdomain() {
	load_plugin_textdomain( 'ddevitos-wp-admin-css', false, dirname( plugin_basename( SCCSS_FILE ) ) . '/languages/' );
}
...
function sccss_register_codemirror( $hook ) {
	if ( 'appearance_page_simple-custom-css' === $hook ) {
		wp_enqueue_style( 'codemirror-css', plugins_url( 'ddevitos-wp-admin-css/codemirror/codemirror.css' ) );
		wp_enqueue_script( 'codemirror-js', plugins_url( 'ddevitos-wp-admin-css/codemirror/codemirror.js' ), array(), '20140329', true );
		wp_enqueue_script( 'codemirror-css-js', plugins_url( 'ddevitos-wp-admin-css/codemirror/css.js' ), array(), '20140329', true );
	}
}
...
function sccss_register_submenu_page() {
	add_theme_page( __( 'Simple Custom CSS', 'ddevitos-wp-admin-css' ), __( 'Custom CSS', 'ddevitos-wp-admin-css' ), 'edit_theme_options', basename( SCCSS_FILE ), 'sccss_render_submenu_page' );
}
...
function sccss_render_submenu_page() {
	...
	$content = isset( $options['sccss-content'] ) && ! empty( $options['sccss-content'] ) ? $options['sccss-content'] : __( '/* Enter Your Custom CSS Here */', 'ddevitos-wp-admin-css' );

	if ( isset( $_GET['settings-updated'] ) ) : ?>
		<div id="message" class="updated"><p><?php _e( 'Custom CSS updated successfully.', 'ddevitos-wp-admin-css' ); ?></p></div>
	<?php endif; ?>
	<div class="wrap">
		<h2 style="margin-bottom: 1em;"><?php _e( 'Simple Custom CSS', 'ddevitos-wp-admin-css' ); ?></h2>
		<form name="sccss-form" action="options.php" method="post" enctype="multipart/form-data">
			...
			<div id="templateside">
				...
				<p style="margin-top: 0"><?php _e( 'Simple Custom CSS allows you to add your own styles or override the default CSS of a plugin or theme.', 'ddevitos-wp-admin-css' ) ?></p>
				<p><?php _e( 'To use, enter your custom CSS, then click "Update Custom CSS".  It\'s that simple!', 'ddevitos-wp-admin-css' ) ?></p>
				<?php submit_button( __( 'Update Custom CSS', 'ddevitos-wp-admin-css' ), 'primary', 'submit', true ); ?>
